    ...
    <div className="myPhotos container container-box-bg mt-15">
      {!profilePictureState?.url && (
        <div className="flex justify-center align-center">
          <UploadImage
            isMultiple={true}
            multipleRefetch={refetch}
          ></UploadImage>
        </div>
      )}

      {profilePictureState?.url && (
        <div className="grid grid-cols-2 grid-cols-2-responsive grid-gap-20 border-1 rounded-10 p-15">
          <UploadImage
            fullWidth={true}
            isMultiple={true}
            multipleRefetch={refetch}
          ...
          <ProfileSettings
            ...
            setProfilePictureBlank={() => setProfilePicture({})}
            refetch={refetch}
          ></ProfileSettings>
        </div>
      )}
      ...
      <div className="text-center w-75 m-auto mt-15 w-md-100-responsive">
        <Alert icon={<IconAlertCircle size="1rem" />} color="pink">
          Note: You can upload 20 photos to your profile. Each photos must be
          less than 15 MB and in jpg, jpeg, png or webp format.
        </Alert>
      </div>
      ...
      <div className="profile-img--container flex flex-gap-15 flex-wrap">
            ...
            <Image
              ...
              caption={
                <div className="flex flex-gap-5 align-center">
                  <ThemeIconComp
                    iconComp={<IconCircleCheck size="14" />}
                    size={18}
                  ></ThemeIconComp>
        ...
        {data?.data?.length > 0 ? (
          data?.data?.map((item) => (
            <Box
              onClick={() => handleProfileImage(item)}
              key={item?._id}
              maw={220}
            >
              <Image
