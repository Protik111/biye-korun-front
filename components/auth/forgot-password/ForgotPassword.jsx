...
import LoginSlider from "../Login/LoginSlide";
...
const ForgotPassword = () => {
  ...
  const handleUpdatePassword = () => {
    if (valiDateFormPass()) {
      ...
      setPassLoading(true);
      axios
        .patch("user/resetpassword", payloadForReset)
        .then((res) => {
      ...
      return (
        <div className="flex flex-gap-5 flex-wrap align-center">
          <Badge
            ...
            size="lg"
          >
            Resend OTP in ({hours}:{minutes}:{seconds})
          </Badge>
        </div>
      );
    }
  ...
  } = useAxiosPost("user/otp-send", null, messageUpdate);
  ...
  const handleSendOTP = () => {
    ...
    if (validateForm()) {
      ...
      setLoading(true);
      axios
        .patch("user/forgotpassword", payload)
        .then((res) => {
          setLoading(false);
          if (res.data?.success) {
            setIsOTPSend(true);
            ...
            <>
              <div className="accountSettings__top">
                ...
                <PasswordInput
                  ...
                  onChange={(event) =>
                    handleChangeFormValues("newPassword", event.target.value)
                  }
              ...
              <br />
              <Link href={"/login"}>
                <div className="flex flex-gap-5 align-center pointer">
                  <BsArrowLeft color="#6F747C" />
                  <p style={{ color: "#6F747C" }}>Back to Sign In</p>
                </div>
              </Link>
            </>
          ) : (
